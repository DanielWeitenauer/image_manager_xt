...
* UI Verbesserungen:
** Klick auf den Namen eines Bildtyps öffnet dessen Effekt-Einstellungen (und nicht mehr die Beschreibung des Bildtypes - diese ist über das linke Iconzeile erreichbar)
...
*1.* @IMAGE_MANAGER_INIT@ : Dieser EP wird sehr früh im Verlauf des IM Codes aufgerufen (in der _config.inc.php_), er erlaubt Zugriff auf folgende Variablen im übergebenen @$params@ array: @rex_img_type@. @rex_img_file@, @imagepath@, @cachepath@

*2.* @IMAGE_MANAGER_FILTERSET@ : In diesem EP (in der _class.rex_image_manager.inc.php_) wird der @image_type@ samt dem für ihn definierten Filterset übergeben. D.h. man kann an dieser Stelle die für diesen Bildtyp definierten Filter und ihre Parameter bearbeiten, als auch weitere hinzufügen, bzw. ganz neue erstellen. Weiterhin kann man an dieser Stelle ad hoc einen "virtuellen" image_type samt Filterset erzeugen (über diesen Weg wird z.b. die rex_resize/rex_filter Funktionalität des _rex_resize.imagemanager.plugin_ im IM nachgebildet).

h2. Repo

"https://github.com/jdlx/image_manager_ep":https://github.com/jdlx/image_manager_ep
